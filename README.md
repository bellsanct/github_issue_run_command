...
## 注意事項
- GitHub CLI (`gh`) をインストールし、`gh auth status` で認証済みであること。
...
3. Claude のコマンドとして `/github_issue_run {issue_number}` を呼び出します。

### Codex で使う場合
1. `.codex/prompts` が空の場合は作成。
2. `for_codex/prompts/github_issue_run.md` を `.codex/prompts/github_issue_run.md` に配置。
3. `.codex/commands` に含まれるサンプルコマンド（`echo`, `hello` など）はそのまま利用できます。必要に応じて `commands/` にある元ファイルをコピーして編集してください。
4. Codex では `/prompts:github_issue_run` を実行し、案内に従って Issue 番号を入力すると自動化が始まります。
